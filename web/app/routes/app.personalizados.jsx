...
function buildOrderData(order, shopDomain) {
  ...
  // Fallback para pedidos com PE1198 sem os atributos do modal (inclusão por
  // vendedor externo): descreve os itens do pedido para a tarefa não ir vazia.
...
export const action = async ({ request, context }) => {
  try {
    ...
    for (const node of orders) {
      ...
      const payload = {
        name: `AGRO | Personalizado ${od.name}${od.hasAttributes ? "" : " (sem atributos)"}`,
        status: CU_STATUS,
...
export default function Personalizados() {
  ...
  const filteredOrders = useMemo(() => {
    switch (filter) {
      case "enviados": return orders.filter((o) => sent[o.id]);
      case "pendentes": return orders.filter((o) => !sent[o.id]);
      case "semAtributos": return orders.filter((o) => !o.hasAttributes);
              ...
              <InlineStack align="space-between" blockAlign="center">
                <InlineStack gap="300" blockAlign="center">
                  ...
                  <Text as="span" variant="bodySm" tone="subdued">
                    {filteredOrders.length} pedido(s) · {pendingCount} pendente(s){selectedResources.length > 0 ? ` · ${selectedResources.length} selecionado(s)` : ""}
                  </Text>
                </InlineStack>
                <InlineStack gap="200">
                  <Button
